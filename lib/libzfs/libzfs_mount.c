...
proto_table_t proto_table[PROTO_END] = {
	...
	{ZFS_PROP_SHAREAOE, "aoe", EZFS_SHAREAOEFAILED, EZFS_UNSHAREAOEFAILED},
};
...
boolean_t
zfs_is_shared_aoe(zfs_handle_t *zhp, char **where)
{
	return (zfs_is_shared_proto(zhp, where,
	    PROTO_AOE) != SHARED_NOT_SHARED);
}
...
int
zfs_share_aoe(zfs_handle_t *zhp)
{
	return (zfs_share_proto(zhp, aoe_only));
}
...
int
zfs_unshare_aoe(zfs_handle_t *zhp, const char *mountpoint)
{
	return (zfs_unshare_proto(zhp, mountpoint, aoe_only));
}
...
int
zfs_unshareall_aoe(zfs_handle_t *zhp)
{
	return (zfs_unshareall_proto(zhp, aoe_only));
}
